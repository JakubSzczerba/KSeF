...
namespace Ksef\Api\Infrastructure\Ksef\Api;

use Ksef\Api\Application\Contract\KsefApi;
use Ksef\Api\Infrastructure\Exception\ApiClientException;
...
final class KsefApiClient implements KsefApi
{
    ...
    ): array
    {
        ...
        $response = $this->httpClient->request('GET', self::API_URL . '/sessions', [
            'headers' => [
                ...$headers,
            ],
            'query' => [
    ...
    ): array
    {
        ...
        $response = $this->httpClient->request(
            'GET',
            self::API_URL . '/sessions/' . rawurlencode($sessionReferenceNumber) . '/invoices',
            [
                'headers' => [
                    ...$headers,
                ],
    ...
    public function downloadInvoiceByKsefNumber(string $accessToken, string $ksefNumber): array
    {
        $response = $this->httpClient->request(
            'GET',
            self::API_URL . '/invoices/ksef/' . rawurlencode($ksefNumber),
            [
                'headers' => [
                    'Authorization' => 'Bearer ' . $accessToken,
        ...
        $hash = null;
        if (isset($headers['x-ms-meta-hash'][0]) && is_string($headers['x-ms-meta-hash'][0])) {
